...
<?php
$cek = mysqli_query($koneksi,"SELECT * FROM student_mahasiswa WHERE nim='$_username' AND semester == 5");
...
$cek2 = mysqli_query($koneksi,"SELECT * FROM krs_mbkm WHERE nim='$_username' AND status = 'y'");

$row2 = mysqli_num_rows($cek2);
if (empty($row2)){
    echo "<script>window.alert('MAAF HALAMAN TIDAK BISA DIAKSES')
		    window.location.href='{$_url}krs-mbkm'</script>";
}
...
if ($acc['accept']==0):
    ?>
<a href= "<?= $_url ?>krs-mbkm/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
<?php 

else:
    ?>
<a target="_BLANK" href="<?= $_url ?>krs-mbkm/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
<?php
endif;
?>
<?php
	
// $sql = "SELECT akademik_krs.*, akademik_tahun_akademik.keterangan, akademik_konsentrasi.nama_konsentrasi, makul_matakuliah.kode_makul, 
//         makul_matakuliah.nama_makul, makul_matakuliah.sks, app_dosen.nama_lengkap,makul_kelompok.kode_kelompok FROM
//         akademik_krs
...
//         WHERE akademik_krs.nim='$_username' AND akademik_krs.konversi='$semester'";
  ...
  width="100%">
	...
	?>
		<tr>
			...
			<td><?php
			$quququ = mysqli_query($koneksi,"Select accept from akademik_krs where nim='$_username' and konversi='$semester'");
			...
			<div class="inline-block">
					...
					<li><a href="<?= $_url ?>krs-mbkm/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a></li>
					</ul>
			</div>
			<?php
			else:
