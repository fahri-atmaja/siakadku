...
if (isset($_params[3]) && $_params[3] == 'yes') {
	...
	if ($query) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt-konversi/view/{$_id}'; }, 1500);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
...
<a href="<?= $_url ?>krs-kpt-konversi/delete/<?= $_id ?>/<?= $_params[1] ?>/<?= $_params[2] ?>/yes" class="button primary">Yes</a>
<a href="<?= $_url ?>krs-kpt-konversi/view/<?= $_id ?>" class="button danger">No</a>