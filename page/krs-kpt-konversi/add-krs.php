<?php
	if ($_access == 'mahasiswa' && $_id != $_username) {
		header("location:{$_url}krs-kpt-konversi/view/{$_username}");
	}
...
<h1>
<a href="<?= $_url ?>krs-kpt-konversi/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt-konversi/view/<?= $_username ?>'; }, 2000);
		</script>";
	    } else {
	...
	if($num > 0){
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt-konversi/view/<?= $_id ?>'; }, 2000);
		</script>";
	}else{
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt-konversi/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
				...
				WHERE student_mahasiswa.nim='$_username' and akademik_tahun_akademik.status='y' and makul_matakuliah.kelompok_id ='3' and
				jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')
				ORDER BY makul_matakuliah.kelompok_id ASC, akademik_jadwal_kuliah.semester ASC, akademik_jadwal_kuliah.id_kelas ASC");
				...
				echo '<option name="jadwal_id" value="' . $row['jadwal_id'] . '">Kelas:' . $row['nama_kelas'] . ' - Semester: ' . $row['semester'] . '-' . $row['kode_makul'] . '-' . $row['nama_makul'] . '</option>';
