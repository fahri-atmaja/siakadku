...
if (isset($_POST['submit'])) {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-konversi/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
				...
				WHERE student_mahasiswa.nim='$_username' and akademik_tahun_akademik.status='y' and makul_matakuliah.kelompok_id != '1' and
