...
if ($validasi['kpt']=='1'){
    echo "<script>window.alert('ANDA TERDAFTAR DIKELAS KPT.')
    window.location.href='{$_url}krs-kpt-konversi/view/$_username'</script>";
}
?>
<style type="text/css">
	.input-control {
...
<?php
 ...
 $loadkrs = mysqli_query($koneksi,"SELECT sum(mm.sks) as value_sks FROM makul_matakuliah as mm, akademik_krs as ak, akademik_jadwal_kuliah as jk 
			WHERE mm.makul_id=jk.makul_id AND jk.jadwal_id=ak.jadwal_id AND ak.nim='$_username' AND semester='$semester' AND ak.konversi!='1'");
...
$loadkpt = mysqli_query($koneksi,"SELECT angkatan_id,kpt FROM student_mahasiswa WHERE nim='$_username'");
$kondisi= mysqli_fetch_array($loadmhs);
$acc = mysqli_fetch_array($accept);
$kpt = mysqli_fetch_array($loadkpt);

if ($kpt['kpt']=='0' and $kpt['angkatan_id']<='20' and $kondisi['status']=='1'):
	?>	
	<!--<a href= "<?= $_url ?>krs-konversi/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>-->
		<a target="_BLANK" href="<?= $_url ?>krs-konversi/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
		<?php
...
<?php
elseif ($kpt['kpt']=='0' and $kpt['angkatan_id']<='20' and $acc['accept']=='1'):
    echo "<script type='text/javascript'>alert('Dimohon untuk klik Bayar SKS untuk melanjutkan akademik. Terimakasih');</script>";
?>

<a href= "<?= $_url ?>krs-konversi/bayar/<?= $_id ?>/<?= urlencode($semester) ?>"><button type="submit" name=submit>Bayar SKS</button></a>
<?php
else:
	?>
	<a href= "<?= $_url ?>krs-konversi/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
...
?>
	...
	$sql    = "SELECT akademik_krs.*, akademik_kelas.nama_kelas, makul_matakuliah.kode_makul, 
    	    makul_matakuliah.nama_makul, akademik_tahun_akademik.keterangan, makul_matakuliah.sks, app_dosen.nama_lengkap
    	    ,makul_kelompok.kode_kelompok 
    	    FROM
            akademik_krs
...
// 			$rowe= mysqli_fetch_array($quququ);
			...
			<div class="inline-block">
				    <a href="<?= $_url ?>krs-konversi/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a>
			</div>
			<?php
			else:
			?>
	...
	<tfoot><tr><td colspan='5' align='right'>Total SKS yang ditempuh</td><td><?php 
...
if (isset($_POST['submit'])) {
    ...
    window.location.href='{$_url}krs-konversi/bayar/$_username'</script>";
	}else{
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-konversi/bayar'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
