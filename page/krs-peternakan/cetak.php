...
if ($validasi['id_sks']!='1'){
    echo "<script>window.alert('MAAF HALAMAN TIDAK BISA DIAKSSES')
		    window.location.href='{$_url}'</script>";
}
// $semester = $_params[1];
// $cekdulu = mysqli_num_rows(mysqli_query($koneksi,"SELECT * from bayar_sks_peternakan where nim = '$_username' and semester ='$semester' and status=1"));
// //echo $mhs['status'];
// if ($cekdulu > 0):
 	?>
...
?>
<?php
// echo "<script>window.alert('MAAF ANDA BELUM MELAKUKAN PEMBAYARAN SKS')
