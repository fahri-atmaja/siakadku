...
<h1>
<a href="<?= $_url ?>krs-peternakan/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-peternakan/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-peternakan/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
                ...
                <?php
				$result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*,student_mahasiswa.semester as sem, akademik_konsentrasi.nama_konsentrasi, 
				makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, makul_matakuliah.makul_id as makul_id,makul_matakuliah.kelompok_id,makul_kelompok.kode_kelompok,
				app_dosen.nama_lengkap as nama_lengkap, akademik_kelas.nama_kelas
				...
				jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username') 
				ORDER BY akademik_jadwal_kuliah.semester ASC, makul_matakuliah.kelompok_id ASC");
				...
				echo '<option name="jadwal_id" value="' . $row['jadwal_id'] . '">Kurikulum: ' . $row['kode_kelompok'] . ' - Kelas:' . $row['nama_kelas'] . ' - Semester: ' . $row['semester'] . '-' . $row['kode_makul'] . '-' . $row['nama_makul'] . '</option>';
