<?php
	$cek = mysqli_query($koneksi,"SELECT konsentrasi_id,id_sks FROM student_mahasiswa WHERE nim='$_username'");
$validasi = mysqli_fetch_array($cek);
if ($validasi['konsentrasi_id']!=23){
...
<?php 
if($angkatan_id < 17){
$cekdulu = mysqli_num_rows(mysqli_query($koneksi,"SELECT * from bayar_sks_peternakan where nim = '$nim' and semester ='$semester' and status=1"));
$accept = mysqli_query($koneksi,"SELECT accept FROM akademik_krs WHERE nim='$_username' and konversi='$semester'");
$loadmhs = mysqli_query($koneksi,"SELECT status FROM bayar_sks_peternakan WHERE nim='$_username' and semester='$semester'");
$kondisi= mysqli_num_rows($loadmhs);
$acc = mysqli_fetch_array($accept);

if ($acc['accept']==1 AND $cekdulu == 0 AND $field['beasiswa']!=1):
?>
<a href= "<?= $_url ?>krs-peternakan/bayar/<?= $_id ?>/<?= urlencode($semester) ?>"><button type="submit" name=submit>Bayar SKS</button></a>
<p><span style="color: red">*Pastikan SKS yang ditempuh sudah benar <br>karena setelah klik bayar anda tidak bisa mengulangi KRS lagi</span></p>
<?php
elseif ($acc['accept']==1 AND $cekdulu > 0):
?>
<a href= "<?= $_url ?>krs-peternakan/cetak/<?= $_id ?>" class="button success">Cetak</a>
<?php
elseif ($acc['accept']==1 AND $field['beasiswa']==1):
?>
<a href= "<?= $_url ?>krs-peternakan/cetak/<?= $_id ?>" class="button success">Cetak</a>
<?php
else:
	?>
	<a href= "<?= $_url ?>krs-peternakan/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
	<br>
<span style="color: red">Konsultasikan kepada dosen wali. Jika KRS sudah selesai dipilih,<br>Silahkan meminta persetujuan KRS kepada fakultas/dosen wali<br> 
lalu melakukan pembayaran ke BAUK dengan klik "BAYAR SKS" terlebih dahulu<br> </span>
<br>
...
<?php
	...
	$sql    = "SELECT akademik_krs.*, akademik_kelas.nama_kelas, akademik_tahun_akademik.keterangan,
	        makul_matakuliah.kode_makul, makul_matakuliah.nama_makul, makul_matakuliah.sks, app_dosen.nama_lengkap,makul_kelompok.kode_kelompok FROM
            akademik_krs
...
// 			$rowe= mysqli_fetch_array($quququ);
			...
			<div class="inline-block">
				    <a href="<?= $_url ?>krs-peternakan/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a>
			</div>
			<?php
			else:
			?>
...
if (isset($_POST['submit'])) {
	$cekdulu = mysqli_num_rows(mysqli_query($koneksi,"SELECT * from bayar_sks_peternakan where nim = '$nim' and semester ='$semester'"));
	if ($cekdulu > 0){
		echo "<script>window.alert('ANDA SUDAH KRS, SILAHKAN MELAKUKAN PEMBAYARAN')
    window.location.href='{$_url}krs-peternakan/bayar/$_username'</script>";
	}else{
	...
	$sqlsks 		 = "INSERT INTO bayar_sks_peternakan SET nim='$postnim', semester='$postsemester', id_sks='$jenis', jumlah_sks='$jmlsks', jumlah_bayar='$bayar_sks'";
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-peternakan/bayar/$_username'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
