...
<h1>
<a href="<?= $_url ?>krs-konversi-briva/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	if ($sumsks['value_sks']+$sks > 24){
		echo "<script>window.alert('MAAF SKS MELEBIHI SYARAT! SYARAT MAKSIMAL 24 SKS')
		    window.location.href='{$_url}krs-konversi-briva/view/$_username'</script>";
	}else{
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-konversi-briva/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-konversi-briva/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
                ...
                <?php
                $result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*, akademik_konsentrasi.nama_konsentrasi, makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, 
                makul_matakuliah.makul_id as makul_id, makul_kelompok.kode_kelompok, app_dosen.nama_lengkap as nama_lengkap, akademik_kelas.nama_kelas
				...
				WHERE student_mahasiswa.nim='$_username' AND akademik_tahun_akademik.status='y' and makul_matakuliah.kelompok_id !='1' and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");
				$jsArray = "var prdName = new Array();\n";
				while ($row= mysqli_fetch_array($result)) {
				echo '<option name="jadwal_id" value="' . $row['jadwal_id'] . '">' . $row['kode_kelompok'] . ' - Semester: ' . $row['semester'] . '-' . $row['kode_makul'] . '-' . $row['nama_makul'] . '-' . $row['nama_kelas'] . '</option>';
