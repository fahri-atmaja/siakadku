...
$loadmakul = mysqli_query($koneksi,"SELECT mm.nama_makul FROM akademik_jadwal_kuliah as jk, makul_matakuliah as mm WHERE mm.makul_id=jk.makul_id AND jk.jadwal_id='$_id' AND jk.dosen_id='$dosen_id'");
$makul = mysqli_fetch_array($loadmakul);
?>
<a href="<?= $_url ?>presensi_mahasiswa" class="nav-button transform"><span></span></a>
ABSENSI SUSULAN <?php echo $makul['nama_makul'] ?>
</h1>
<!--<p style="color : red">ABSENSI SUSULAN AKAN DITUTUP SETELAH MID SEMESTER.-->
<!--<BR>MOHON UNTUK SEGERA MENGISIKAN ABSENSI SUSULAN MAHASISWA<br> DILANJUTKAN MENGISI JURNAL DOSEN UNTUK MENAMBAH "PERTEMUAN" </p>-->
<?php
$nimshow = $_params[1];
$querya = mysqli_query($koneksi,"select jk.*, akrs.jadwal_id, sm.nama, mm.nama_makul,ah.hari as day FROM
			akademik_krs as akrs, student_mahasiswa as sm, makul_matakuliah as mm, akademik_jadwal_kuliah as jk,app_hari as ah WHERE
			jk.hari_id=ah.hari_id and sm.nim=akrs.nim and jk.makul_id=mm.makul_id and jk.jadwal_id=akrs.jadwal_id and akrs.jadwal_id='$_id' and akrs.accept=1 and jk.dosen_id='$dosen_id'");
...
$hari = $fieldd['day'];
echo "Jadwal hari "; echo "<b>" . $hari . "</b>";
$hitung = $pertemuan - $perjalan + 1 ;
$cekpertemuan = mysqli_query($koneksi,"SELECT * FROM mhs_absen WHERE jadwal_id='$_id' AND pertemuan='$hitung'");
	if(mysqli_num_rows > 0){
	    echo "<script>window.alert('Maaf anda sudah melakukan presensi!')
    window.location='{$_url}absen_dosen/absen/$_id'</script>";
	}
...
<div>
    <input type="date" name="tanggalabsen" id="tanggalabsen" value="" required>
</div>
	...
	$no=1;
 	...
 	?>
		...
		<select name="kehadiran[]" id="kehadiran[]" value="" required>
			<!--<option value="not set">--Kehadiran--</option>-->
...
if (isset($_POST['submit'])) {
	...
	$sql2= "INSERT into mhs_absen (absen_id,jadwal_id,nim,pertemuan,tanggalabsen,jamabsen,kehadiran) values('','$_id','$nim[$i]','$hitung','$tanggalabsen','$jamabsen','$kehadiran[$i]')";
