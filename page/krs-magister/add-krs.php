<?php
	if ($_access == 'mahasiswa' && $_id != $_username) {
		header("location:{$_url}krs-magister/view/{$_username}");
	}
?>
<?php
$cek = mysqli_query($koneksi,"SELECT kpt,id_sks,status_angsur, FROM student_mahasiswa WHERE nim='$_username'");
$validasi = mysqli_fetch_array($cek);
if ($validasi['kpt']=='1'){
    echo "<script>window.alert('MAAF HALAMAN TIDAK BISA DIAKSSES')
		    window.location.href='{$_url}'</script>";
}
elseif  ($validasi['id_sks']=='2'){
    echo "<script>window.alert('MAAF HALAMAN TIDAK BISA DIAKSSES')
		    window.location.href='{$_url}'</script>";
}
elseif  ($validasi['status_angsur']=='1'){
    echo "<script>window.alert('MAAF HALAMAN TIDAK BISA DIAKSSES')
		    window.location.href='{$_url}'</script>";
}
?>
...
<h1>
<a href="<?= $_url ?>krs-magister/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
$querya = mysqli_query($koneksi, "SELECT student_mahasiswa.*, akademik_kelas.nama_kelas, biaya_sks.id_sks,biaya_sks.jenis_sks, student_angkatan.keterangan as angkatan, akademik_konsentrasi.nama_konsentrasi as nama_konsentrasi FROM student_mahasiswa 
...
if (isset($_POST['submit'])) {
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-magister/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-magister/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
                ...
                <?php
    //             if ($semester!=3){
				// $result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*, akademik_tahun_akademik.keterangan, akademik_konsentrasi.nama_konsentrasi, makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, makul_matakuliah.makul_id as makul_id, app_dosen.nama_lengkap as nama_lengkap
				...
				// and makul_matakuliah.kelompok_id!='1' and akademik_jadwal_kuliah.semester<3
				// and akademik_tahun_akademik.status='y' and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");
    //             }else{
                $result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*, makul_kelompok.kode_kelompok, akademik_tahun_akademik.keterangan, akademik_konsentrasi.nama_konsentrasi, makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, makul_matakuliah.makul_id as makul_id, app_dosen.nama_lengkap as nama_lengkap
				FROM akademik_jadwal_kuliah 
				LEFT JOIN makul_matakuliah ON akademik_jadwal_kuliah.makul_id=makul_matakuliah.makul_id
				...
				LEFT JOIN akademik_tahun_akademik ON akademik_tahun_akademik.tahun_akademik_id=akademik_jadwal_kuliah.tahun_akademik_id
				WHERE student_mahasiswa.nim='$_username'
				and makul_matakuliah.kelompok_id !='1'
				and akademik_tahun_akademik.status='y' and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");  
                // }
				$jsArray = "var prdName = new Array();\n";
				while ($row= mysqli_fetch_array($result)) {
				echo '<option name="jadwal_id" value="' . $row['jadwal_id'] . '">Kurikulum:' . $row['kode_kelompok'] . '- Semester: ' . $row['semester'] . '-' . $row['kode_makul'] . '-' . $row['nama_makul'] . '</option>';
...
$loaduang=mysqli_query($koneksi,"SELECT jenis_bayar_id, semester FROM keuangan_pembayaran_detail WHERE nim='$_username' AND jenis_bayar_id=3 ORDER BY jenis_bayar_id");
if (mysqli_num_rows($loaduang)){
while ($status= mysqli_fetch_array($loaduang)){
if ($status['jenis_bayar_id']=='3' and $status['semester']=='1' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='1'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='2' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='2'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='3' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='3'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='4' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='4'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='5' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='5'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='6' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='6'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
elseif ($status['jenis_bayar_id']=='3' and $status['semester']=='7' and $kondisi['status_angsur']=='0' and $kondisi['semester']=='7'):
?>
	<div class="cell">
		<button type="submit" name="submit" class="button primary">SUBMIT</button>
<?php
