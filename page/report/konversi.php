...
if(isset($_GET['bulan']))
{
				$bulan = $_GET['bulan'];
				$tahun = $_GET['tahun'];
				$sql1 = "SELECT bs.*, sm.nama FROM bayar_sks as bs, student_mahasiswa as sm 
			 	WHERE sm.nim=bs.nim and month(bs.tanggal_bayar)='$bulan' and year(bs.tanggal_bayar)='$tahun'";
...
<table class="table striped hovered border bordered">
	...
	<?php
		...
		if (mysqli_num_rows($query1) > 0):
			while($field1 = mysqli_fetch_array($query1)):
				$total = $total+$field1['jumlah_bayar'];
	?>
		<tr>
			<td><?= $no++ ?></td>
			...
			<td><?= $field1['status']==1?'Lunas':'Belum lunas'; ?></td>
		</tr>
	<?php
			endwhile;
		else:
	...
	<tfoot><tr><td colspan='7' align='right'>Total</td><td><?php 
			echo $total ?></td></tr>
