...
if ($acc['accept']==0):
    ?>
<a href= "<?= $_url ?>krs-briva/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
<?php 

else:
    ?>
<a target="_BLANK" href="<?= $_url ?>krs-briva/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
<?php
endif;
?>
<?php
	...
	?>
		<tr>
			...
			<div class="inline-block">
					...
					<li><a href="<?= $_url ?>krs-briva/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a></li>
					</ul>
			</div>
			<?php
			else:
