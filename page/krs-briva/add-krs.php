...
<h1>
<a href="<?= $_url ?>krs-briva/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	if ($sumsks['value_sks']+$sks > 24){
		echo "<script>window.alert('MAAF SKS MELEBIHI SYARAT! SYARAT MAKSIMAL 24 SKS')
		    window.location.href='{$_url}krs-briva/view/$_username'</script>";
	}else{
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-briva/view/$_id'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-briva/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
		...
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
				...
				WHERE student_mahasiswa.nim='$_username' and akademik_jadwal_kuliah.semester=student_mahasiswa.semester
				 AND akademik_tahun_akademik.status='y' and makul_matakuliah.kelompok_id !='1' AND akademik_jadwal_kuliah.id_kelas=student_mahasiswa.id_kelas and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");
