...
<h1>
<a href="<?= $_url ?>krs-kerjasama-konversi/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	if ($sumsks['value_sks']+$sks > 24){
		echo "<script>window.alert('MAAF SKS MELEBIHI SYARAT! SYARAT MAKSIMAL 24 SKS')
		    window.location.href='{$_url}krs-kerjasama-konversi/view/$_username'</script>";
	}else{
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kerjasama-konversi/view/<?= $_username ?>'; }, 2000);
		</script>";
	    } else {
	...
	if($num > 0){
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kerjasama-konversi/view/<?= $_id ?>'; }, 2000);
		</script>";
	}else{
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kerjasama-konversi/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
