<?php
	if ($_access == 'mahasiswa' && $_id != $_username) {
		header("location:{$_url}krs-kerjasama-konversi/view/{$_username}");
	}
	$cek = mysqli_query($koneksi,"SELECT id_sks,beasiswa FROM student_mahasiswa WHERE nim='$_username'");
...
// 		    window.location.href='{$_url}krs-kerjasama-konversi/view/$username'</script>";
// }
?>
<style type="text/css">
	.input-control {
...
<!-- <a href= "<?= $_url ?>krs-kerjasama-konversi/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a> -->
<?php 
...
if ($kpt['kpt']=='0' and $kondisi['status']=='1'):
	...
	<!--<a href= "<?= $_url ?>krs-konversi/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>-->
		<a target="_BLANK" href="<?= $_url ?>krs-kerjasama-konversi/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
		<?php
...
?>
<a href= "<?= $_url ?>krs-kerjasama-konversi/bayar/<?= $_id ?>/<?= urlencode($semester) ?>"><button type="submit" name=submit>Bayar SKS</button></a>
<?php
else:
	?>
	<a href= "<?= $_url ?>krs-kerjasama-konversi/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
...
?>
		<?php
	$sql    = "SELECT akademik_krs.*, akademik_kelas.nama_kelas, akademik_tahun_akademik.keterangan, makul_matakuliah.kode_makul, 
...
// 			$rowe= mysqli_fetch_array($quququ);
			...
			<div class="inline-block">
				    <a href="<?= $_url ?>krs-kerjasama-konversi/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a>
			</div>
			<?php
			else:
			?>
...
if (isset($_POST['submit'])) {
    ...
    window.location.href='{$_url}krs-kerjasama-konversi/bayar/$_username'</script>";
	}else{
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kerjasama-konversi/bayar'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
