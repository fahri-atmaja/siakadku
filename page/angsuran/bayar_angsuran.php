...
if ($cekdulu > 0){
	...
	if ($que) {
		...
		setTimeout(function(){ window.location.href='{$_url}angsuran/bayar_angsuran/$_id'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
...
<?php
$sql1 = "SELECT sm.*, ba.bayar_id, ba.bayar, ba.angsuran, ba.tanggal FROM student_mahasiswa as sm, bayar_angsuran as ba 
		...
		<tr>
			...
			<td>
				<div class="inline-block">
					<a href="<?= $_url ?>angsuran/angsuran-delete/<?= $field1['bayar_id'] ?>" class="place-right"><span class="mif-cross"> Hapus </span></a>
				</div>
