...
<h1>
<a href="<?= $_url ?><?= in_array($_access, array('fakultas')) ? 'transkrip_fakultas' : '' ?>" class="nav-button transform"><span></span></a>
...
extract($field);
// if($id_sks==2){
//         header("location:{$_url}transkrip_fakultas/konversi/{$nim}");
//     }
?>
...
<a target="_BLANK" href="<?= $_url ?>transkrip_fakultas/transkrip-sementara/<?= $_id ?>" class="button success">Cetak</a>
...
<?php
		...
		$sql = "select ak.*,ah.*,mm.makul_id,mm.nama_makul,mm.sks,ad.nama_lengkap
                            ...
                            WHERE ah.krs_id=ak.krs_id and mm.makul_id=jk.makul_id and ad.dosen_id=jk.dosen_id and jk.jadwal_id=ak.jadwal_id and ak.nim='$nim'";
