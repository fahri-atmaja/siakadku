...
<?php
$cek = mysqli_query($koneksi,"SELECT kpt FROM student_mahasiswa WHERE nim='$_username'");
...
if ($acc['accept']==1):
    ?>
<a target="_BLANK" href="<?= $_url ?>krs-kpt/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
<?php
else:
?>
<a href= "<?= $_url ?>krs-kpt/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
<?php
endif;
?>
<?php
	...
	?>
		<tr>
			...
			<div class="inline-block">
					...
					<li><a href="<?= $_url ?>krs-kpt/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a></li>
					</ul>
			</div>
			<?php
			else:
