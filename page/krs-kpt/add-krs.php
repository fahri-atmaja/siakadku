<?php
	if ($_access == 'mahasiswa' && $_id != $_username) {
		header("location:{$_url}krs-kpt/view/{$_username}");
	}
$cek = mysqli_query($koneksi,"SELECT kpt, konsentrasi_id, semester FROM student_mahasiswa WHERE nim='$_username'");
...
<h1>
<a href="<?= $_url ?>krs-kpt/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
...
if (isset($_POST['submit'])) {
	...
	if ($sumsks['value_sks']+$sks > 24){
		echo "<script>window.alert('MAAF SKS MELEBIHI SYARAT! SYARAT MAKSIMAL 24 SKS')
		    window.location.href='{$_url}krs-kpt/view/$_username'</script>";
	}else{
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-kpt/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
                ...
                <?php
                if ($validasi['konsentrasi_id']==23 AND $validasi['semester']>=2){
				$result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*, akademik_konsentrasi.nama_konsentrasi, makul_kelompok.kode_kelompok, makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, makul_matakuliah.makul_id as makul_id,makul_matakuliah.kelompok_id , app_dosen.nama_lengkap as nama_lengkap
				...
				WHERE student_mahasiswa.nim='$_username' AND 
				akademik_tahun_akademik.status='y' AND akademik_jadwal_kuliah.id_kelas=student_mahasiswa.id_kelas
				...
				WHERE student_mahasiswa.nim='$_username' AND 
				akademik_tahun_akademik.status='y' AND akademik_jadwal_kuliah.id_kelas=student_mahasiswa.id_kelas
				and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username') and
				akademik_jadwal_kuliah.semester='$semester'
				ORDER BY akademik_jadwal_kuliah.semester ASC"); 
                }
