<?php
$cek = mysqli_query($koneksi,"SELECT id_sks,kpt,beasiswa,konsentrasi_id,semester FROM student_mahasiswa WHERE nim='$_username'");
...
// }
// elseif ($validasi['semester']>1 AND $validasi['konsentrasi_id']==23){
...
<?php
 ...
 $loadkrs = mysqli_query($koneksi,"SELECT sum(mm.sks) as value_sks FROM makul_matakuliah as mm, akademik_krs as ak, akademik_jadwal_kuliah as jk 
			WHERE mm.makul_id=jk.makul_id AND jk.jadwal_id=ak.jadwal_id AND ak.nim='$_id' AND ak.konversi='$semester' ");
			$query13 = mysqli_query($koneksi,"SELECT accept FROM akademik_krs WHERE nim='$_id' AND konversi='$semester'");
...
// if ($row['aktif_krs']=="y"){
$loadmhs = mysqli_query($koneksi,"SELECT status_angsur,semester FROM student_mahasiswa WHERE nim='$_id'");
$kondisi= mysqli_fetch_array($loadmhs);
$loaduang=mysqli_query($koneksi,"SELECT jenis_bayar_id FROM keuangan_pembayaran_detail WHERE nim='$_id' AND jenis_bayar_id >= 34 AND jenis_bayar_id <= 40 ORDER BY jenis_bayar_id");
...
//             and jk.semester='$semester'";
    $sql = "SELECT akademik_krs.*, akademik_tahun_akademik.keterangan, makul_matakuliah.kode_makul, makul_matakuliah.nama_makul, makul_matakuliah.sks, app_dosen.nama_lengkap, akademik_kelas.nama_kelas FROM
        akademik_krs
        ...
        WHERE akademik_krs.nim='$nim' AND akademik_krs.konversi='3'";
	...
	?>
		<tr>
			...
			<div class="inline-block">
					...
					<li><a href="<?= $_url ?>krs-susulan/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a></li>
					</ul>
			</div>
			<?php
			else:
...
<input type="hidden" name="nim" value="<?php echo $_id?>">
