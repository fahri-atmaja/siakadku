...
if (isset($_POST['submit'])) {
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
				...
				and makul_matakuliah.kelompok_id !='1'  AND akademik_jadwal_kuliah.id_kelas=student_mahasiswa.id_kelas and
				akademik_tahun_akademik.status ='y' and
				akademik_jadwal_kuliah.jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");
