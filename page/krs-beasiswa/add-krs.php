<?php
	if ($_access == 'mahasiswa' && $_id != $_username) {
		header("location:{$_url}krs-beasiswa/view/{$_username}");
	}
	$cek = mysqli_query($koneksi,"SELECT beasiswa FROM student_mahasiswa WHERE nim='$_username'");
...
<h1>
<a href="<?= $_url ?>krs-beasiswa/view/<?= $_id ?>" class="nav-button transform"><span></span></a>ADD KRS
</h1>
<?php
...
if (isset($_POST['submit'])) {
	...
	$valid      = mysqli_num_rows($cekjdwl);
		...
		setTimeout(function(){ window.location.href='{$_url}krs-beasiswa/view/<?= $_id ?>'; }, 2000);
		</script>";
	    } else {
	...
	if ($queque) {
		...
		setTimeout(function(){ window.location.href='{$_url}krs-beasiswa/add-krs/<?= $_id ?>'; }, 2000);
		</script>";
	} else {
		echo "<script>$.Notify({
		    caption: 'Failed',
	...
	<label>JADWAL PRODIMU</label>
		<div class="form-group">
			<select class="form-control" name="jadwal_id" id="jadwal_id" onchange='changeValue(this.value)' required>
                ...
                <?php
                $result=mysqli_query($koneksi,"SELECT akademik_jadwal_kuliah.*, makul_kelompok.kode_kelompok, akademik_konsentrasi.nama_konsentrasi, makul_matakuliah.kode_makul, makul_matakuliah.sks as sks,makul_matakuliah.nama_makul as nama_makul, makul_matakuliah.makul_id as makul_id, app_dosen.nama_lengkap as nama_lengkap
				...
				and makul_matakuliah.kelompok_id !='1' AND akademik_tahun_akademik.status='y' AND akademik_jadwal_kuliah.id_kelas=student_mahasiswa.id_kelas and jadwal_id NOT IN (Select jadwal_id FROM akademik_krs Where nim='$_username')");
				$jsArray = "var prdName = new Array();\n";
				while ($row= mysqli_fetch_array($result)) {
				echo '<option name="jadwal_id" value="' . $row['jadwal_id'] . '">' . $row['kode_kelompok'] . ' - Semester: ' . $row['semester'] . '-' . $row['kode_makul'] . '-' . $row['nama_makul'] . '</option>';
