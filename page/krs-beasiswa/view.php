...
$loadkrs = mysqli_query($koneksi,"SELECT sum(mm.sks) as value_sks FROM makul_matakuliah as mm, akademik_krs as ak, akademik_jadwal_kuliah as jk 
			WHERE mm.makul_id=jk.makul_id AND jk.jadwal_id=ak.jadwal_id AND ak.nim='$_username' AND ak.semester='$semester' ");
...
if ($acc['accept'] != 1):
    ?>
<a href= "<?= $_url ?>krs-beasiswa/add-krs/<?= $_id ?>" class="button primary">Pilih Matakuliah</a>
<?php 
else:
?>
<a target="_BLANK" href="<?= $_url ?>krs-beasiswa/cetak/<?= $_id ?>/<?= urlencode($semester) ?>" class="button success">Cetak</a>
<?php
endif;
?>
<?php
	
// 	$sql = "SELECT akademik_krs.*, akademik_tahun_akademik.keterangan, makul_matakuliah.kode_makul, makul_matakuliah.nama_makul, makul_matakuliah.sks, app_dosen.nama_lengkap FROM
//         akademik_krs
...
$sql = "SELECT akademik_krs.*, akademik_tahun_akademik.keterangan, makul_matakuliah.kode_makul, makul_matakuliah.nama_makul, makul_matakuliah.sks, makul_kelompok.kode_kelompok, app_dosen.nama_lengkap FROM
        akademik_krs
        LEFT JOIN akademik_jadwal_kuliah ON akademik_jadwal_kuliah.jadwal_id=akademik_krs.jadwal_id
        LEFT JOIN makul_matakuliah ON makul_matakuliah.makul_id=akademik_jadwal_kuliah.makul_id
        LEFT JOIN app_dosen ON app_dosen.dosen_id=akademik_jadwal_kuliah.dosen_id
	...
	?>
		<tr>
			...
			<div class="inline-block">
					...
					<li><a href="<?= $_url ?>krs-beasiswa/delete/<?= $_id ?>/<?= $field['krs_id'] ?>/<?= urlencode($field['nim']) ?>" class="place-right"><span class="mif-cross"> Hapus KRS </span></a></li>
					</ul>
			</div>
			<?php
			else:
