          ...
          <form method="POST">
            <div class="form-row">
              <div class="col-12 col-md-9 mb-2 mb-md-0">
                <input type="text" name="email" class="form-control form-control-lg" placeholder="Enter your email..." value="<?= $view['email']; ?>">
              </div>
              <div class="col-12 col-md-3">
                ...
                ?>
                <a href="https://siakad.undaris.ac.id/update-email.php"><label>Bukan Email Anda?</label></a>
                <?php
                endif;
                ?>
              </div>
...
function kirim_email($email, $code){
        ...
        <div class="container">
            ...
            <center>
                ...
                <h2>Kode Verifikasi Anda : <a href="https://siakad.undaris.ac.id/verifikasi.php?code='.$code.'" target="_blank">'.$code.'</a></h2>
                <h3> Gunakan Strip "-"</h3>
            </center>
            </div>
        </div>
...
$code=$angkastring;
    $email = $_POST['email'];
    $cek = mysqli_query($koneksi,"SELECT * FROM email_verifikasi WHERE nim='$_username'");
    if (mysqli_num_rows($cek) > 0){
        $cek = mysqli_fetch_array($cek);
        $code = $cek["code"];
        $change_email = mysqli_query($koneksi,"UPDATE email_verifikasi SET email = '$email' WHERE nim='$_username'");
        kirim_email($email, $code);
        echo "<script>window.alert('Email Sudah dikirim ulang! Silahkan verifikasi cek kode di inbox anda.');</script>";
    }else{
